...
const donateProgress = document.querySelectorAll('.donation__cont__bar__progress');
const donateMoney = document.querySelectorAll('.donation__cont__money');
const todayMoney = document.querySelectorAll(".today__money");
const todayPeople = document.querySelectorAll(".today__people");
...
const increaseNum = (startTime, currentNum, maxNum, money, totalTime) => {
    ...
    money.innerText = currentDisplayNum.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");

    if (progress < 1) { // If progress is less than 100%
        requestAnimationFrame(() => increaseNum(startTime, currentNum, maxNum, money, totalTime));
    }
}
insertNum();